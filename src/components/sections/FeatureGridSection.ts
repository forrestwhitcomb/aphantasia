  ...
  const cards = features
    .map(
      (f) => {
        ...
        const ctaHtml = f.cta
          ? `\n      <a href="${esc(f.ctaHref || '#')}" class="aph-btn-accent aph-btn-sm" style="margin-top:12px;">${esc(f.cta)}</a>`
          : "";
        return `<div class="aph-feature-card${f.imageSrc ? " aph-feature-card--has-image" : ""}">
      ${visual}
      <h3 class="aph-feature-heading">${esc(f.heading || "Feature")}</h3>
      <p class="aph-feature-body">${esc(
        f.body || "A short, focused description of this capability."
      )}</p>${ctaHtml}
    </div>`;
      }
    )
